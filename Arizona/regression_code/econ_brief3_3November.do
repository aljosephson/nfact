...
** TRIAL FIGURE 6** 
	tabstat 	FOOD_SECURITY_since [aw=WEIGHTS] if job_anychange==1, by(race_temp) stat(mean N)
	tabstat 	FOOD_SECURITY_since [aw=WEIGHTS] if job_anychange==0, by(race_temp) stat(mean N)
...
*is the var that comes up wierd for me. Shows means of 14.89 for No job change and 4.49 for yes job change. */
* 75 variables are 88 which is a dne - fixed in updated file 
