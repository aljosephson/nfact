...
* load data
	*clear
	import excel "Arizona data (numbers_ -99_ multi-value fields NOT split) - (8_31_20)", sheet("Sheet0") firstrow case(lower)
	*** put in file name
...
* want to use screener data for weights, income, etc.					
	...
	label var				scrn_lived_AZ "Screen: Lived in AZ since 1/1/2020"  /* NOT LABELED */
...
clonevar scrn_genderid = qs7
	label var				scrn_genderid "Screen: Gender Identity"             /* NOW LABELED */ 	
...
tab qs7 scrn_genderid, miss
	...
	label var				scrn_hisplat_origin "Screen: Hispanic, Latino, or Spanish Origin" /* NOT LABELED */
tab scrn_hisplat_origin, miss
		
	
*	rename 					qs9 scrn_race
clonevar scrn_race = qs9
	label var				scrn_race "Screen: Race"							/* NOW LABELED */
tab scrn_race, miss
...
bysort scrn_hisplat_origin: tab RACE scrn_race, miss	
	...
	label var				text_scrn_race "Screen: Text: Race"					/* NOW LABELED */
...
tostring usda_foodlast_covid usda_afford_covid, replace
	...
	foreach v of varlist usda_foodlast_covid usda_afford_covid{
		...
		replace 		`v' = 99 if `v'_temp == "99"
		replace 		`v' = . if `v'_temp == "-99"
		label def 		`v'_lab2_2 1 "Never true" 2 "Sometimes true" 3 "Often true" ///		/* label name was `v'_lab2_1 */
							99 "Do not know"
...
tostring usda_eatless* usda_cutskip* usda_hungry*, replace	
	...
	foreach v of varlist usda_eatless* usda_cutskip* usda_hungry*{
		...
		replace 		`v' = 99 if `v'_temp == "99"
		replace 		`v' = . if `v'_temp == "-99"
		label def 		`v'_lab2_3 1 "Yes" 0 "No" ///							/* label name was `v'_lab2_1 */
							99 "Do not know" 									/* This line was from the previous label (3 "Often true") */
...
/* What is this variable for? --> See lines [630-650]
	   
	   gen 				prog_nouse = .
	   replace 			prog_nouse = 1 if prog_snap == 4
	   replace 			prog_nouse = 1 if prog_wic == 4
	   replace 			prog_nouse = 1 if prog_school == 4
	   replace 			prog_nouse = 1 if prog_pantry == 4
	   replace 			prog_nouse = 0 if prog_snap != 4 & prog_snap != .  
	   replace 			prog_nouse = 0 if prog_wic != 4 & prog_wic != .  
	   replace 			prog_nouse = 0 if prog_snap != 4 & prog_snap != .  
	   replace 			prog_nouse = 0 if prog_school != 4 & prog_school != .  
	   replace 			prog_nouse = 0 if prog_pantry != 4 & prog_pantry != .  
*/	
...
tostring snap_*, replace 
	...
	foreach v of varlist snap_* {    
	   ...
	   replace			`v' = . if `v'_temp == "-99"
	   replace 			`v' = .a if prog_snap == 4 | prog_snap == 2				/* ADDED | prog_snap == 2 */ 
...
tostring wic_*, replace
	...
	foreach v of varlist wic_* {    
	   ...
	   replace 			`v' = 5 if `v'_temp == "5" 
	   replace			`v' = . if `v'_temp == "-99"
	   replace 			`v' = .a if prog_wic == 4 | prog_wic == 2				/* ADDED | prog_wic == 2 */ 
...
foreach v of varlist school_* {    
	   ...
	   replace 			`v' = .a if prog_school == 4 | prog_school == 2				/* ADDED | prog_school == 2 */  
	   replace			`v' = . if `v'_temp == "-99"
...
foreach v of varlist pantry_* {    
	   ...
	   replace 			`v' = 5 if `v'_temp == "5" 
	   replace			`v' = . if `v'_temp == "-99"
	   replace 			`v' = .a if prog_pantry == 4 | prog_pantry == 2				/* ADDED | prog_pantry == 2 */   
...
* NEW CODE to create a binary variable indicating households who did not use ANY of these programs prior/since covid 
...
* Let's create a new variable to indicate 'any change'  
gen job_anychange = . 
...
/* I think there are issues with this section --> Let me know if you want to review it together
...
*************************************************	
/* We don't have these questions in the AZ survey 
...
/* This code had one mistake: the 6 for '2 or more' was then recoded into 'Asians' -- See my code below for the 'race' variable
...
tab race_temp, miss	
	...
	recode 					race_temp (12 = 1) (3 = 2) (1 = 3) (2 5 6 7 8 11 = 4) (4 9 10 = 5) (13 = 6) , generate(race) /* Took out (. = 14), put 9,10 as PIs, combined other and multiple */
	label var				race "Race"
...
* **********************************************************************
* 9 - WEIGHTS (based on income quintile)
* **********************************************************************	
...
replace WEIGHTS = 0.060/(55/620)  if scrn_income==1
replace WEIGHTS = 0.131/(81/620)  if scrn_income==2
replace WEIGHTS = 0.229/(165/620) if scrn_income==3
replace WEIGHTS = 0.192/(112/620) if scrn_income==4
replace WEIGHTS = 0.131/(106/620) if scrn_income==5
replace WEIGHTS = 0.257/(101/620) if scrn_income==6	
