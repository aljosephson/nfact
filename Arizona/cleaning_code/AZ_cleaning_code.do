...
*	rename					qs7 scrn_genderid
	...
	label 					define scrn_hisplat_origin  0 "0=not hispanic, latino, spanish" 1 "1=hispanic, latino, spanish" 
...
* race + ethnicity 
	...
	label 					define race_ethnicity  0 "0=Hispanic" 1 "1=NH white" 2 "2=NH Black" ///
								3 "3=Asian" 4 "4=Native American" 5 "5=Other/Multiple"	
...
* education 	
	...
	label 					define education   1 "1=Less than HS" 2 "2=HS" 3 "3=Some college" 4 ///
								"4=Associates degree" 5 "5=College degree" 6 "6=More advanced degree"	
...
* only respond if you did cut the size of your meals (per usda_cutskip*)
	...
	replace	usda_cutskip_year = 1 if usda_oftencut_year == 1 | usda_oftencut_year == 2 | usda_oftencut_year == 3 
	tab usda_cutskip_year usda_oftencut_year, miss		
...
*** SINCE COVID ***	
	...
	tab1 				usda_foodlast_covid usda_afford_covid usda_eatless_covid ///
							usda_cutskip_covid usda_hungry_covid usda_oftencut_covid, miss
...
* create a binary variable indicating households who did not use ANY of these programs prior/since covid 
	...
	gen 				prog_nouse_prior = .
	label var 			prog_nouse_prior "Used ANY food assistance programs prior to covid" 
	replace 			prog_nouse_prior = 0 if prog_snap_prior==0 & prog_wic_prior==0 & ///
							prog_school_prior==0 & prog_pantry_prior==0 & prog_other_prior==0 		
	replace 			prog_nouse_prior = 1 if prog_snap_prior==1 | prog_wic_prior==1 | ///
							prog_school_prior==1 | prog_pantry_prior==1 | prog_other_prior==1 
	label 				values prog_nouse_prior yesno 
	tab 				prog_nouse_prior, miss	

	gen 				prog_nouse_since = .
	label var 			prog_nouse_since "Used ANY food assistance programs since covid" 
	replace 			prog_nouse_since = 0 if prog_snap_since==0 & prog_wic_since==0 & ///
							prog_school_since==0 & prog_pantry_since==0 & prog_other_since==0 		
	replace 			prog_nouse_since = 1 if prog_snap_since==1 | prog_wic_since==1 | ///
							prog_school_since==1 | prog_pantry_since==1 | prog_other_since==1 
	label 				values prog_nouse_since yesno 
	tab 				prog_nouse_since, miss	
...
foreach v of varlist foodprog_* {    
	   ...
	   replace 			`v' = 5 if `v'_temp == "5" 
	   replace			`v' = . if `v'_temp == "-99"
	   replace 			`v' = .a if prog_nouse_since == 0 				
...
/* I think there are issues with this section --> Let me know if you want to review it together - okay sounds good 
